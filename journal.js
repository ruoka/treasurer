...
export const account = () => {
    ...
    general_ledger.balance_sheet.forEach(ledger => balances.set(ledger.account, sum(ledger.account)));
    general_ledger.income_statement.forEach(ledger => balances.set(ledger.account, sum(ledger.account)));
    // Calculate summary balances
    balances.set('1000', sum('1'));
    balances.set('2000', sum('2') + sum('3') + sum('4') + sum('5'));
    balances.set('2130', sum('3') + sum('4') + sum('5') + sum('9999'));
    balances.set('3100', sum('31'));
    balances.set('3200', sum('32'));
    balances.set('3300', balances.get('3100') + balances.get('3200'));
    balances.set('4100', sum('41'));
    balances.set('4200', sum('42'));
    balances.set('4300', balances.get('4100') + balances.get('4200'));
    balances.set('5100', sum('51'));
    balances.set('5200', sum('52'));
